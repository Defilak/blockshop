...
function prefixview(){
document.getElementById('prefixview').value = '['+document.getElementById('pref').value+']';
document.getElementById('prefixview').style.background = document.getElementById('prefcol').style.background;
}
function doIcon(s1)
 {
  ...
  div1.value = s1;
  if(s1!=s1.split('_')[0]){div2.value = s1.split('_')[0];}
  else{div2.value = s1.split('.')[0];}
 }
 function addench(){
...
function valuta()
 {
  ...
  if(div.value == 0) {div.value++;} else {div.value--;}
...
function zedit() {
	...
	var g = '::';
	ajaxfunc('edit='+b1+g+b2+g+b3+g+b4+g+b5+g+b10+g+b6+g+b7+g+b8+g+b9);
}
function del(s1) {
	ajaxfunc('del='+s1);
	$("#m"+s1).fadeOut(500);
