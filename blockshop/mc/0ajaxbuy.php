...
function banlist() {
    global $banlist, $db;
    $q = $db->select("SELECT * FROM `{$banlist}`");
    for ($u = 0; $u < count($q); $u++) {
...
///склад////
function cart($s1) {
    global $cart, $dir, $cartdesign, $s, $db, $goodly;
...
///история///
function history($s1) {
    global $dir, $logs, $historydesign, $db, $goodly;
...
function giveskin() 
{
    ...
    if (file_exists($path_skin.'/'.$username.'.png')) 
    {
...
function removeskin($s1) {
    ...
    if ($s1 == 'skin') {
        unlink($path_skin.$username.'.png');
        unlink($path_skin.'1/'.$username.'.png');
        unlink($path_skin.'2/'.$username.'.png');
        goodly("Скин удален!");
    } else {
        unlink($path_cloak.$username.'.png');
        goodly("Плащ удален!");
    }
}
...
if (isset($_FILES['cloak'])) {
    ...
    if (($image_info[0] == 64 AND $image_info[1] == 32) || ($image_info[0] == 128 AND $image_info[1] == 64) || ($image_info[0] == 256 AND $image_info[1] == 128) || ($image_info[0] == 512 AND $image_info[1] == 256) || ($image_info[0] == 1024 AND $image_info[1] == 512)) {
        ...
        if (move_uploaded_file($_FILES['cloak']['tmp_name'], $path_cloak.$username.'.png')) {
            exit();
        }
    } else {
        exit();
...
if (isset($_FILES['skin'])) {
    ...
    if (($image_info[0] == 64 AND $image_info[1] == 32) || ($image_info[0] == 128 AND $image_info[1] == 64) || ($image_info[0] == 256 AND $image_info[1] == 128) || ($image_info[0] == 512 AND $image_info[1] == 256) || ($image_info[0] == 1024 AND $image_info[1] == 512)) {
        ...
        if (!move_uploaded_file($_FILES['skin']['tmp_name'], $path_skin.$username.'.png')) {
            die('Ошибка записи');
        }
        ...
        imagepng($fullsize, $path_skin.'1/'.$user_name.'.png');
        ...
        imagefill($fullsize, 0, 0, $transparent);
        imagecopyresized($fullsize, $preview, 0, 0, 0, 0, imagesx($fullsize), imagesy($fullsize), imagesx($preview), imagesy($preview));
        imagepng($fullsize, $path_skin.'2/'.$user_name.'.png');
...
function admin($s1) {
    global $dir, $blocks, $admlist, $admbox, $admcont, $db, $asd;
    if (!ctype_digit($s1))
