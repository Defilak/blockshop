...
if($ik_sign_hash === $sign_hash) {
	$ex->update("UPDATE `{$eco['0']}` SET `{$eco['3']}`=`{$eco['3']}`+? WHERE `{$eco['1']}`='{$ik_payment_id}'"); 
	$ex->update("INSERT INTO `transactions` (`name`, `date`, `money`) VALUES (`{$ik_payment_id}`,`".date('Y-m-d H:i:s')."`,`{$ik_payment_amount}`)");
} else {
exit;
}
?>